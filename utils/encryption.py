...
# Все API-ключи Ozon (Client ID и API Key) шифруются этим ключом перед сохранением в БД.
# Это гарантирует, что даже при утечке дампа базы данных злоумышленник не сможет 
# использовать ключи пользователей без доступа к файлу .env и ENCRYPTION_KEY.
...
def encrypt_credential(plaintext: str) -> str:
    ...
    Использует симметричное шифрование Fernet. Данные преобразуются в байты, 
    шифруются и возвращаются в виде строки base64.
...
def decrypt_credential(ciphertext: Optional[str]) -> Optional[str]:
    ...
    Использует мастер-ключ ENCRYPTION_KEY для обратного преобразования данных.
    Если ключ в .env был изменен, расшифровка существующих в БД данных станет невозможной.
